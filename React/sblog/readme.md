...
# 11강 input
- <input />에 뭔가 입력 시 코드 실행하고 싶으면 inChange/onInput
- onClick은 클릭시, 타이핑 입력시는 onChange
- onMouseOver={}, onScroll..등등 이벤트 핸들러 매우 많음

1. <input>에 입력한 값 가져오는 법
- e(이벤트 관련한 기능이 담겨있음)라는 파라미터 넣기
e.target => 이벤트 발생한 html태그 : 여기서 input태그 
e.target.value => input에 입력한 값
저장하려면 보통 변수 또는 state에 저장함
- ** state변경함수는 늦게 처리됨 : 비동기처리
      <input
        onChange={(e) => { 
          입력값변경(e.target.value); //이거 처리되기전에
          console.log(입력값); //이거 먼저 처리됨
        }} />

2.이벤트버블링 
<div>
    <h4>
        <span onClick={}></span> => 여기서 클릭하면, span, h4, div 총3번 클릭됨
        이벤트가 상위 요소로 퍼지는 이벤트 버블링 현상!
        이벤트 핸들링 막으려면? e.stopPropagation() : JS문법
    </h4>
</div>

# 12강 : 응용
1. 글에 아무것도 입력 안하고 발행버튼 누르는 거 막기
-> 삼항 연산자 참일 경우 조건 여러개 못하나..? ㅠㅠ ,도 ;도 ()도 써봤는데 안된다..
2. 글을 하나 추가하면 따봉갯수 개별적용하던것 이상해짐
-> 따봉 기록할 곳 추가하기
3. 현재 날짜로 기록하기
